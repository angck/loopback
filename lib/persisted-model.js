...
module.exports = function(registry) {
  ...
  PersistedModel.setupRemoting = function() {
    ...
    setRemoting(PersistedModel, 'subscribe', {
      description: 'Subscribe to changes to the model.',
      accessType: 'READ',
      http: [
        {verb: 'post', path: '/subscription'},
        {verb: 'get', path: '/subscription'}
      ],
      accepts: {
        arg: 'options',
        type: 'object'
  ...
  /**
   * Subscribe to changes of this model.
   *
   * @param {Object} options
  ...
  PersistedModel.subscribe = function(options, cb) {
    if(typeof options === 'function') {
      cb = options;
      options = undefined;
    }
    ...
    function createChangeHandler(type, optmistic) {
      return function(ctx, next) {
        // we don't support optmistic create yet
        if(ctx.isNewInstance && optmistic) {
          return next();
        }
